...
# atn-sim
from adsb.adsb_in import forwarder_factory
...
class MlatServer(object):
    ...
    def __init__(self, config="aimod.cfg"):
        ...
        @param f_config: configuration file
        """
        self.dbname = "mlat"
        self.dbpass = "mlat"
        self.dbuser = "mlat"
        ...
        # Remove old messages
        rows = cursor.execute("DELETE FROM messages WHERE created < now()")
    ...
    def start(self):
        ...
        while True:
            ...
            # Select next message
            nrows = cursor.execute("SELECT id, message, sensor_id, toa, created FROM messages ORDER BY created ASC LIMIT 1")
            if nrows > 0:
    ...
    def process_msg(self, message, delete=False):
        ...
        # query1 = "SELECT id, sensor_id, toa, created, tx_xpos, tx_ypos, tx_zpos FROM messages WHERE message='%s' ORDER BY created ASC LIMIT %s" % (message, self.nsensors)
        query1 = "SELECT id, sensor_id, toa, created FROM messages WHERE message='%s' ORDER BY created ASC LIMIT %s" % (message, self.nsensors)
        ...
        if nrows < self.nsensors:
            ...
            if delete:
                for i in msg_id:
                    cursor.execute("DELETE FROM messages WHERE id=%s" % i)
                    print delete
